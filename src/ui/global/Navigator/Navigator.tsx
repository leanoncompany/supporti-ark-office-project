...
import { Box, AppBar, Toolbar, IconButton, Typography, Breakpoint, useMediaQuery, Container } from '@mui/material';
import MenuIcon from '@mui/icons-material/Menu';
import { useTheme } from '@mui/material';
import { useLocation } from 'react-router';
import { NavigatorCore, DrawerPanel, DropdownMenu, DropdownButton, Link } from '@leanoncompany/supporti-react-ui';
...
const Navigator = (props: INavigatorProps) => {
  ...
  const toolbarContents: { [key: string]: React.ReactElement } = {
    spacing: <Box key={`${generateRandomNumber()}-spacing`} sx={{ flexGrow: 1 }} />,
    drawer: (
      <DrawerPanel
        key={'drawer'}
        drawerRadius={0}
          ...
          <Box display="flex" alignItems={'center'}>
            ...
            {props.logoImage !== undefined ? (
              ...
              <Box>
                <img
                  style={{ width: '30px' }}
                  src={props.logoPath !== undefined ? props.logoPath : '/images/logo/newLogo.png'}
                />
              </Box>
            )}
          </Box>
    ...
    rightMenu: (
      <Box key={'rightMenu'} display={'flex'} alignItems={'center'}>
        {props.rightMenus !== undefined &&
          props.rightMenus.map((el, index) => <Box mr={props.rightMenus!.length - 1 !== index ? 3 : 0}>{el}</Box>)}
      </Box>
    ),
  };
  ...
  const toolbar: { [key: string]: React.ReactElement } = navigatorCore.setToolbarByOrder(props.order, toolbarContents);

  //* Media query hooks
  const isBetweenXsAndSm = useMediaQuery(themes.breakpoints.between('xs', 'sm'));
  const isBetweenSmAndMd = useMediaQuery(themes.breakpoints.between('sm', 'md'));
  const isBetweenMdAndLg = useMediaQuery(themes.breakpoints.between('md', 'lg'));
  const isBetweenLgAndXl = useMediaQuery(themes.breakpoints.between('lg', 'xl'));
  const isUpperXl = useMediaQuery(themes.breakpoints.up('xl'));
  ...
  React.useEffect(() => {
    ...
    for (const [breakpointKey, breakpointStatus] of Object.entries(breakpointOrder)) {
      if (breakpointStatus) {
        setBreakpoint(breakpointKey as Breakpoint);
        break;
      }
    }
  }, [isBetweenXsAndSm, isBetweenSmAndMd, isBetweenMdAndLg, isBetweenLgAndXl, isUpperXl]);
      ...
      return (
        <Container
          ...
          sx={{
            ...
            px: props.containerPaddingX !== undefined ? { md: props.containerPaddingX, xs: 0 } : undefined,
          }}>
          <Toolbar
            disableGutters={true}
            sx={{
  ...
  return (
    <Box display={props.disableHeader !== true ? 'block' : 'none'} mb={props.subMenu !== undefined ? '60pt' : '45pt'}>
      <AppBar
        position="fixed"
        sx={{
        ...
        {props.subMenu !== undefined && (
          <Container
            ...
            sx={{
              maxWidth: props.containerMaxWidth !== undefined ? props.containerMaxWidth : undefined,
              px: props.containerPaddingX !== undefined ? props.containerPaddingX : undefined,
            }}>
            {props.subMenu}
          </Container>
        )}
        <Box px={props.containerPaddingX !== undefined ? props.containerPaddingX : 3}>{checkUseContainer()}</Box>
      </AppBar>
    </Box>
  );
};
