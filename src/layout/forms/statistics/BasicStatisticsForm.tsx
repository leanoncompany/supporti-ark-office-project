...
const BasicStatisticsForm = (props: IBasicStatisticsFormProps) => {
  ...
  const [contentChild, setContentChild] = useState<{
    ...
    CREATED_AT: (element: any) => {
      return (
        <Typography
          sx={{ whiteSpace: "pre-line", fontWeight: 400 }}
          variant={"body2"}
        >
          {moment(element.date).format(
            element.SELECTED_DATE_UNIT === "month" ? "YYYY/MM" : "YYYY/MM/DD"
          )}
        </Typography>
      );
    },
  ...
  useEffect(() => {
    if (startDate !== null && endDate !== null && selectedDateUnit !== null) {
          ...
          statisticsController.getStatisticsList(
            ...
            (response) => {
              ...
              if (props.formatDataList !== undefined) {
                resultDataList = props.formatDataList(resultDataList);
              }
              ...
              if (sumModelKey !== undefined && sumValueKey !== undefined) {
                let totalAmount: number = 0;

                resultDataList.map((el: any) => {
                  if (el.data.hasOwnProperty(sumModelKey)) {
                    el.data[sumModelKey].map((element: any) => {
                      if (sumCallback !== undefined) {
                        totalAmount = sumCallback(totalAmount, element);
                      } else {
                        totalAmount += element[sumValueKey];
                      }
                    });
                  }
                });
        ...
        <Box>
          <ToggleButtonGroup
            ...
            onChange={(event: React.MouseEvent<HTMLElement>, value: string) => {
              if (!(value === null && selectedDateUnit !== null)) {
            ...
            {dateUnitOptions.map((dateUnitOption) => (
              <ToggleButton
                ...
                sx={{
                  ...
                  "&:hover": {
                    backgroundColor:
                      selectedDateUnit === dateUnitOption.value
