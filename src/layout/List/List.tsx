            ...
            {props.listHeader.map((item, index) => (
              <Grid2 item md={item.gridMd} xs={item.gridXs} key={index}>
                <Box
                  ...
                  alignItems={"center"}
                  justifyContent={
                    props.textAlign !== undefined ? props.textAlign : "start"
                  }
                >
                  {item.icon !== undefined && <Box mr={1}>{item.icon}</Box>}
                  ...
                  <Box>
                    <Typography
                      ...
                      sx={{
                        fontWeight:
                          props.headerFontWeight !== undefined
                ...
                onClick={() => {
                  if (props.modelIdKey !== undefined) {
                    if (props.disableRoute !== true) {
                      let targetLink = `${router.pathname}/${
                        fixedEl[props.modelIdKey]
                      }`;

                      if (props.linkCallback !== undefined) {
                        targetLink = props.linkCallback(fixedEl);
                      } else {
                    ...
                    (listFixedHeaderElement: any, i: number) => {
                      return (
                        <Grid2
                          ...
                          md={listFixedHeaderElement.gridMd}
                          xs={listFixedHeaderElement.gridXs}
                        >
                          <Box pl={i == 0 ? 1 : 0}>
                            {renderColumn(
                              props.fixedRender,
        ...
        {props.data.length == 0 ? (
          ...
          >
            {props.data.map((element: any, index: number) => {
              return props.customListItemRenderCallback !== undefined ? (
                props.customListItemRenderCallback(
                  element,
                  index,
                  props.listHeader,
                  (columnKey: string, element: any) => {
                    return renderColumn(props.render, columnKey, element);
                  }
                )
              ) : (
                <Box
                  ...
                  sx={{
                    "&:hover": {
                      backgroundColor:
                        props.headerBackgroundColor !== undefined &&
                        props.disableRoute !== true
                  ...
                  onClick={() => {
                    if (props.disableOnClick !== true) {
                      if (props.modelIdKey !== undefined) {
                        if (props.disableRoute !== true) {
                          let targetLink = `${router.pathname}/${
                            element[props.modelIdKey]
                          }`;

                          if (props.linkCallback !== undefined) {
                            targetLink = props.linkCallback(element);
                          } else {
                            router.push(targetLink);
                          }
                        }
