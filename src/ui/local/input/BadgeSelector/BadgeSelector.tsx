...
const BadgeSelector = (props: IBadgeSelectorProps): IBadgeSelectorReturnType => {
  //* Modules
   ...
   * 배지 클릭 시
   */
  const onClickBadgeElement = (selectedItemValue: any) => {
      ...
      <Box
        ...
        {props.selectableItemList.map((selectableItem, index) => (
          <Box
            ...
            py={1}
            borderRadius={1}
            bgcolor={selectedItemList.includes(selectableItem.value) ? theme.palette.primary.main : 'white'}
            ...
            <Typography
              variant={'body1'}
              color={selectedItemList.includes(selectableItem.value) ? 'white' : theme.palette.primary.main}>
