...
const useDataControl = (
  ...
  useEffect(() => {
    const fetchData = async () => {
      const wrappedData = dataControl.createWrappedData(setWrappedDataDict, dataList, pageRole);
          ...
          (response) => {
            for (const [key, value] of Object.entries(response.data.result)) {
              if (wrappedData[key]?.state !== undefined) {
                ...
                console.log(wrappedData[key].state instanceof Date);

                switch (typeof wrappedData[key].state) {
                  case 'object':
                    if (wrappedData[key].state instanceof Date) {
                      if (value !== null) {
                        wrappedData[key].state = value instanceof Date ? value : new Date(value as any);
                      }
                    } else if (wrappedData[key].state === null) {
                    } else if (Object.keys(wrappedData[key].state).length === 0) {
                      wrappedData[key].state = JSON.parse(value as any);
                    } else {
                      wrappedData[key].state = JSON.parse(value as any);
                    }
