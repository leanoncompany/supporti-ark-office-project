    ...
    <Box px={3} py={2} width={"100%"} position={"relative"} ref={ref}>
      ...
      >
        ...
        >
          <Box>
            <Profile
              width={profileWidth !== undefined ? profileWidth : 250}
              {...props.profileProps}
            />
            ...
            <Box display={{ xs: "none", md: "block" }}>
              {props.disableAlarm !== true && (
                <Alarm
                  {...props.alarm}
                  modelName={
                    process.env.NEXT_PUBLIC_AUTH_MODEL_NAME || "AdminMember"
                  }
                />
              )}
            </Box>
      ...
      >
        ...
        {menuSets.map((menuSet, menuSetIndex) => (
          <Box
            width={"100%"}
            key={JSON.stringify(menuSetIndex)}
            display={menuSet.hide === true ? "none" : "block"}
          >
            <Box mb={1.25} mt={3.25}>
              <Typography
                variant={"body2"}
