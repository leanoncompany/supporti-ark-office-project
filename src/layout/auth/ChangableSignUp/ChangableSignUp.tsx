...
const ChangableSignUp = (props: IChangableSignUp) => {
    ...
    return (
      <Box display={"flex"} flexDirection={"column"} mb={data.inputMargin}>
        <Box mb={data.labelMargin}>
          <Typography
            variant={data.labelFontSize}
            sx={{
              fontWeight: data.labelFontWeight,
            }}
          >
            {data.label}{" "}
            {data.require && <span style={{ color: "black" }}>*</span>}
          </Typography>
        </Box>
        <TextTypeInput
          fullWidth
  ...
  return (
    <Box display={"flex"} alignItems={"center"} justifyContent={"center"}>
      <Box
        minWidth={{ md: props.containerWidth, xs: "340px" }}
        maxWidth={{ md: props.containerWidth, xs: "340px" }}
      >
        <Box
          display={"flex"}
          alignItems={"center"}
        ...
        <Box display={"flex"} flexDirection={"column"}>
          {props.stringInputItem.map((inputEl: IInputs) =>
            renderInput(inputEl)
          )}
        </Box>
